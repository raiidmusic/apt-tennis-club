...
- `lib/apt-email.ts` is the only Resend sender. It uses the existing server-only `RESEND_API_KEY`, verified `APT_RESEND_FROM_EMAIL` and management `APT_APPLICATION_TO_EMAIL` variables, adds the required explicit HTTP user agent, filters recipient lists and gives each recipient/event a deterministic Resend idempotency key. No SDK, queue, new environment variable or database table was added.
...
- Remaining integration gate: verify the existing Resend sender domain plus `RESEND_API_KEY`, `APT_RESEND_FROM_EMAIL` and `APT_APPLICATION_TO_EMAIL` in the canonical Production target, then deliberately trigger one controlled event to the management inbox. Until that proof exists, delivery is not represented as live.
