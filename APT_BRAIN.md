...
- Source-publication follow-up (2026-08-12): the authorized GitHub account `raiidmusic` now owns the private repository `apt-tennis-club`. The local audited state was committed as `b8dfa42` (`feat: complete operational member management`) and `origin` points to that private repository. `git push` is blocked because the local Git client cannot inherit the authenticated IAB session (`could not read Username for 'https://github.com'`); create a scoped, revocable GitHub credential or configure SSH only after explicit owner approval. Do not reconnect the Vercel project to the empty remote before this commit is pushed.

## Next decision

APT-004 is complete and APT-021 is locally built but not integrated. The production webhook is saved but disabled; the custom domain is attached but DNS is not yet valid, and no production deployment exists. Real rollout requires, in order: create and validate the two DNS records; approve the one unresolved roster status and the final 35-person active set; approve monthly price and first due date; execute APT-018 containment; validate/apply APT-019 in a non-production target; configure the remaining Supabase/Asaas/CPF secrets in Vercel; connect the current source and deploy the endpoint; prove recadastro, duplicate/timeout and signed webhook paths in Asaas Sandbox; then separately activate the production webhook and authorize the production import and real recurring checkouts. No real athlete, CPF or charge has been created yet.
