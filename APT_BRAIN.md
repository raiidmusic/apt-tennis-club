...
| Deployment | Vercel project `apt-tennis-club` (`prj_gnjjgijeXutKy5vZM5Q0nn9hXnx3`) under `gaagustavo-9339's projects`; six recent previews are Ready; `ASAAS_WEBHOOK_TOKEN` is configured as Sensitive and Production-only; no production deployment exists and the canonical domain currently returns `404 DEPLOYMENT_NOT_FOUND` | Connect the approved Git source, configure the remaining reviewed variables and authorize a production deployment in later tasks |
