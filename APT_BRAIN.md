...
- Production configuration inspection 2026-08-15: the canonical Vercel project has `APT_ADMIN_EMAILS` but no `RESEND_API_KEY`, `APT_RESEND_FROM_EMAIL` or `APT_APPLICATION_TO_EMAIL`; the Resend account has `apttennis.com.br` registered but its DKIM, SPF/MX and optional DMARC records are all `Not Started`. Management notification delivery will therefore use `APT_ADMIN_EMAILS` as soon as the Resend key/from sender are configured, avoiding an unnecessary recipient-variable setup.
- Remaining integration gate: add the three required DNS sending records at Registro.br, verify the sender domain in Resend, save `RESEND_API_KEY` and `APT_RESEND_FROM_EMAIL` in canonical Production, then deliberately trigger one controlled event to the management inbox. Until that proof exists, delivery is not represented as live.
